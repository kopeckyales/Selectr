...
(function(root, factory) {
  ...
  Selectr.prototype.open = function () {
    ...
    if (!this.options.length && this.options.ajaxUrl == false) {
      return false;
    }
  ...
  Selectr.prototype.ajaxSearch = function () {
    var that = this;
    if (that.input.value.length < 3) {
      return false;
    }
    clearTimeout(ajaxTimeout);
    ajaxTimeout = setTimeout(function () {
      util.truncate(that.tree);
      that.setMessage(that.config.messages.searching);
      var formData = new FormData();
      formData.append('term', that.input.value);
      for (var option in that.config.ajaxParameters) {
        if (that.config.ajaxParameters.hasOwnProperty(option)) {
          if (that.config.ajaxParameters[option] instanceof Array) {
            for (var i = 0; i < that.config.ajaxParameters[option].length; i++) {
              formData.append(option + '[]', that.config.ajaxParameters[option][i]);
            }
          } else {
            formData.append(option, that.config.ajaxParameters[option]);
          }
        }
      }
      var request = new XMLHttpRequest();
      request.open("POST", that.config.ajaxUrl, true);
